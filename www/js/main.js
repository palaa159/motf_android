...
motf.init = function() {
    ...
    document.addEventListener('backbutton', function() {
        self.AllHistory.pop();
        if (self.AllHistory.length > 0) {
            motf.goToPage(self.AllHistory[self.AllHistory.length - 1]);
        }
    }, false);
    self = this;
    self.poetScroll = 0;
...
motf.bindEvent = function(id) {
    ...
    switch (id) {
        ...
        case 'pPoetDetail':
            ...
            $('.poemVidAlt').on('tap', function() {
                VideoPlayer.play($('iframe')[0].src.replace('embed/', 'watch?v='));
            });
